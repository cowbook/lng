...
按用户要求执行远程编译与部署正确性检查：
- 已将中英文首页研究报告区所有 `a.report-link` 文案统一为 `↓`，并保持原有属性不变。
- 本地快速构建验证通过（`npm run docs:build:fast`）。
- 准备提交当前工作区待发布改动并推送到 `main`，触发 GitHub Actions Pages 部署流程。
- 推送后将检查远程 Action 运行状态与结果，确认部署是否成功。
...
按需求为市场页图表新增鼠标悬浮指示器与数值提示：

- 在 `MarketWarBoard.vue` 的 SVG 图表中新增 hover 捕获层（`rect`）与交互逻辑。
- 鼠标移动时按当前位置映射到最近日期点，显示：
   - 垂直参考线（crosshair）
   - 各指标当前日期高亮点
   - 提示框（日期 + 各指标数值）
- 提示框数值兼容三种对比模式：
